    ...
    run: (message, args, client) => {
        if (args[0]) {
            ...
            message.channel.send(new Discord.MessageEmbed()
                .setFooter(`** Commande  : ${command.name}**\n\n${command.help.description}\n\nSyntaxe : \`${config.prefix}${command.name}${command.help.syntax ? ` ${command.help.syntax}` : ''}\``))
        }
        else {
            message.channel.send(new Discord.MessageEmbed()
                .setTitle('<a:777275893823438849:803916825917784094> Liste des commandes')
                .addField('pour certaine commandes il faut des perm <a:v_:802935774227726346>', true)
